...
#' }
identify_trajectory_legs <- function(.trjs, .grp_var = .data$ICAO24, .max_gap=3){
    df <- .trjs |>
    dplyr::group_by({{.grp_var}}) |>
    dplyr::arrange(.data$TIME, .by_group = TRUE) |>
    dplyr::mutate(
    # new vars - no prefix required on LHS
       SEQ_ID = dplyr::row_number()
      ,STEP   = difftime(.data$TIME, dplyr::lag(.data$TIME, default = dplyr::first(.data$TIME)), units = "sec")
    #-- add grouping for trajectory segments
     ,  LEG = cumsum( .data$STEP >= .max_gap * 60)    ) |>
     dplyr::ungroup()
  return(df)
}
